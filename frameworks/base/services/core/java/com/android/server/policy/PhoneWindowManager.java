...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    //WizOS <WizWmsDebugger> <huangshuiqiang> <2021-01-04> add for start
    ...
    public static  boolean SHOW_SPLASH_SCREENS = true;
    //WizOS <WizWmsDebugger> <huangshuiqiang> <2021-01-04> add for end

    //WizOS <WizVolumekey> <huangshuiqiang> <2019-01-22> add for start
    private com.wiz.server.policy.WizPwmExt mPwmExt =  
        com.wiz.server.WizSystemServiceFactory.getInstance().makePwmExt();
    //WizOS <WizVolumekey> <huangshuiqiang> <2019-01-22> add for end


    private final static String RETRY_PATH = "/sys/bus/i2c/devices/i2c-4/4-0024/bigobject_state";
    ...
    //WizOS <Wiz_SOS> <guangming> <2020/8/20> for sos begin
    static final int MULTI_PRESS_POWER_SOS = 3;
    //WizOS <Wiz_SOS> <guangming> <2020/8/20> for sos end
    //WizOS <Wiz_QMMI> <zenggz> <2020-11-25> add for start
    static final int MULTI_PRESS_POWER_QMMI = 5;
    //WizOS <Wiz_QMMI> <zenggz> <2020-11-25> add for end
    ...
    //WizOS <Wiz_SOS> <guangming> <2020/8/20> for sos begin
    private boolean mPowerSosSupport = false;
    //WizOS <Wiz_SOS> <guangming> <2020/8/20> for sos end
    ///WizOS <Wiz_QMMI> <zenggz> <2020-11-25> add for start
    private boolean mQmmiSupport = false;
    ///WizOS <Wiz_QMMI> <zenggz> <2020-11-25> add for end
    //WizOS <Wiz_Pin> <zenggz> <2020/11/30> add begin
    private int mScreenLock = 0;
    //WizOS <Wiz_Pin> <zenggz> <2020/11/30> end
    ...
    private static final int MSG_HANDLE_VOLUME_UP = 27;
    private static final int MSG_HANDLE_VOLUME_DOWN = 28;
    ...
    private static String ACTION_QMMITEST_ON = "www.qmmitest.on";
    private static String ACTION_QMMITEST_OFF = "www.qmmitest.off";
    private boolean isQMMITest = false;

    //added by cgaopeng for retail demo mode on 20210521 begin
    private boolean hasDownInDemoMode = false;
    private int mKeyCount = 0;

    private void reset() {
        mKeyCount = 0;
    }
    //added by cgaopeng for retail demo mode on 20210521 end
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_HANDLE_ALL_APPS:
                    launchAllAppsAction();
                    break;
                case MSG_HANDLE_VOLUME_UP:
                    try {
                        getAudioService().adjustSuggestedStreamVolume(AudioManager.ADJUST_RAISE,
                                AudioManager.USE_DEFAULT_STREAM_TYPE, AudioManager.FLAG_SHOW_UI | AudioManager.FLAG_PLAY_SOUND, mContext.getOpPackageName(), TAG);
                    } catch (Exception e) {
                        Log.e(TAG, "Error dispatching volume up in SystemGesture.", e);
                    }
                    break;
                case MSG_HANDLE_VOLUME_DOWN:
                    try {
                        getAudioService().adjustSuggestedStreamVolume(AudioManager.ADJUST_LOWER,
                                AudioManager.USE_DEFAULT_STREAM_TYPE, AudioManager.FLAG_SHOW_UI | AudioManager.FLAG_PLAY_SOUND, mContext.getOpPackageName(), TAG);
                    } catch (Exception e) {
                        Log.e(TAG, "Error dispatching volume down in SystemGesture.", e);
                    }
                    break;
            }
        }
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.Global.getUriFor(
                    ...
                    UserHandle.USER_ALL);
            //WizOS <WizHomeKey> <20200629> add start
            resolver.registerContentObserver(Settings.Secure.getUriFor(
                    "isPowerModelSuper"), false, this,
                    UserHandle.USER_ALL);
            //WizOS <WizHomeKey> <20200629> add end
            //WizOS <Wiz_SOS> <guangming> <2020/8/20> for sos begin
            resolver.registerContentObserver(Settings.Global.getUriFor(
                    "wiz_sos_message"), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.Global.getUriFor(
                    "wiz_sos_dial"), false, this,
                    UserHandle.USER_ALL);
            //WizOS <Wiz_SOS> <guangming> <2020/8/20> for sos end
            //WizOS <Wiz_Pin> <zenggz> <2020/11/30> add begin
            resolver.registerContentObserver(Settings.Secure.getUriFor(
                    "enable_screen_lock"), false, this,
                    UserHandle.USER_ALL);
            //WizOS <Wiz_Pin> <zenggz> <2020/11/30> add end
            updateSettings();
        }
    ...
    public void retry() {
        try {
            ...
            bufWriter = new BufferedWriter(new FileWriter(RETRY_PATH));
            bufWriter.write("0");
            bufWriter.close();
        } catch (IOException e) {
            Log.e(TAG, "can't write the " + RETRY_PATH);
        }
    }


    private void powerPress(long eventTime, boolean interactive, int count) {
     //   Log.w(TAG, "hyhhyhkeyevent");
        ...
        //WizOS <Wiz_QMMI> <zenggz> <2020-11-25> add for end
            ...
            switch (mShortPressOnPowerBehavior) {
                ...
                case SHORT_PRESS_POWER_GO_HOME:
                    //WizOS  <WizSystem> <2020-08-06> modify begin
                    ...
                    //WizOS  <WizSystem> <2020-08-06> modify end
                    break;
    ...
    private void powerMultiPressAction(long eventTime, boolean interactive, int behavior) {
        switch (behavior) {
            ...
            case MULTI_PRESS_POWER_BRIGHTNESS_BOOST:
                ...
                break;
            //WizOS <Wiz_SOS> <guangming> <2020/8/20> for sos begin
            case MULTI_PRESS_POWER_SOS:
                if (mPowerSosSupport){
                    mPwmExt.handleSOS(mContext);
                }
                break;
            //WizOS <Wiz_SOS> <guangming> <2020/8/20> for sos end
        }
    ...
    private void powerLongPress() {
        ...
        //WizOS <Wiz_Pin> <zenggz> <2020/11/30> add begin
        if(mScreenLock == 1){
            Settings.System.putInt(mContext.getContentResolver(), "homeKeyPress", (int)(Math.random()*1000));
            Slog.d(TAG," powerLongPress return, because  mScreenLock = " + mScreenLock);
            return;
        }
        //WizOS <Wiz_Pin> <zenggz> <2020/11/30> add end
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        filter = new IntentFilter();
        filter.addAction(ACTION_QMMITEST_ON);
        filter.addAction(ACTION_QMMITEST_OFF);
        context.registerReceiver(qmmiTestReceiver, filter);
        ...
        //Wizos <WizNightPowerSave> <huangshuiqiang> <2020-09-09> add start
        mPwmExt.initGesturesPointerEventListener(mWindowManagerFuncs, mContext, new com.wiz.server.policy.WizPwmExt.CallbacksImpl());
        //Wizos <WizNightPowerSave> <huangshuiqiang> <2020-09-09> add end
        ...
        mKeyguardDelegate = new KeyguardServiceDelegate(mContext,
                new StateCallback() {
                    ...
                    public void onShowingChanged() {
                        //Wizos <WizNightPowerSave> <huangshuiqiang> <2020-09-09> add
                        mPwmExt.onKeyguardShowChanged(mContext, mKeyguardDelegate == null ? false :mKeyguardDelegate.isShowing());
                        
                        mWindowManagerFuncs.onKeyguardShowingAndNotOccludedChanged();
                    }
                });
        //WizOS WizSystem add 20200922 by zenggz start
        mPwmExt.init(mContext,mWindowManagerFuncs);
        //WizOS WizSystem add 20200922 by zenggz end
    }
    ...
    public void updateSettings() {
        ...
        synchronized (mLock) {
            ...
            //WizOS <Wiz_SOS> <guangming> <2020/8/20> for sos begin
            mPowerSosSupport = Settings.Global.getInt(resolver, "wiz_sos_message", 0) == 1 ||
                    Settings.Global.getInt(resolver, "wiz_sos_dial", 0) == 1 ;
            Slog.d(TAG," updateSettings mPowerSosSupport:"+mPowerSosSupport);
            if (mPowerSosSupport){
                mTriplePressOnPowerBehavior = MULTI_PRESS_POWER_SOS;
            }else {
            ...
            //WizOS <Wiz_Pin> <zenggz> <2020/11/30> add begin
            mScreenLock = Settings.Secure.getInt(resolver, "enable_screen_lock", 0);
            Slog.d(TAG," updateSettings mScreenLock:" + mScreenLock);
            //WizOS <Wiz_Pin> <zenggz> <2020/11/30> add end
    ...
    public StartingSurface addSplashScreen(IBinder appToken, String packageName, int theme,
        ...
        //WizOS WizSystem Modify for Not show Splash Screens begin
        try{
            ApplicationInfo info = mContext.getPackageManager().getApplicationInfo(packageName, 0);
            if ((info.flags & ApplicationInfo.FLAG_SYSTEM) != 0) return null;
        } catch (PackageManager.NameNotFoundException e) {
        }
        //WizOS WizSystem Modify for Not show Splash Screens end
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            if (mUseTvRouting || mHandleVolumeKeysInWM) {
             ...
             //WizOS  <WizVolumekey> <huangshuiqiang> <2020-07-14> add for start
            if(mPwmExt.handleVolumeKey(mContext, event, keyguardOn)){
                return -1;
            }
             //WizOS  <WizVolumekey> <huangshuiqiang> <2020-07-14> add for end
    ...
    void launchHomeFromHotKey(final boolean awakenFromDreams, final boolean respectKeyguard) {
        ...
        if(mScreenLock == 1){
            Settings.System.putInt(mContext.getContentResolver(), "homeKeyPress", (int)(Math.random()*1000));
            Slog.d(TAG," launchHomeFromHotKey return, because  mScreenLock = " + mScreenLock);
            return;
        }
        //WizOS <Wiz_Pin> <zenggz> <2020/11/30> add end

        //add by tdeqin ID1013028 add for handle home event in PostDownloadMessageWindow on 2021-04-22 begin
        ...
        if (activityManager != null) {
            ...
            if (runningTaskInfos != null && runningTaskInfos.size() > 0) {
                ...
                String className = runningTaskInfo.topActivity.getClassName();
                    if ("com.verizon.dmclientupdate.OmadmFumoDialog".equals(className) &&
                                                                        "1".equals(SystemProperties.get("persist.sys.endcall", "0")) &&
                                                                         "1".equals(SystemProperties.get("10df_mandatory_fota","0"))){
                        return;
                    }
            }
        }
        //add by tdeqin ID1013028 add for handle home event in PostDownloadMessageWindow on 2021-04-22 end

        //WizOS WizHomeKey zenggz 20200929 add start
        if(mPwmExt.launchHomeFromHotKey(mContext)){
            return;
        }
        if(SystemProperties.getInt("persist.sys.wizhome.top", 0) == 1 && mScreenOnEarly && mScreenOnFully){
            mPwmExt.startWizAppList(mContext);
            return;
        }
        //WizOS WizHomeKey zenggz 20200929 add end
    ...
    public int finishPostLayoutPolicyLw() {
        ...
        if (mStatusBar != null) {
            ...
            if (mForceStatusBar || mForceStatusBarFromKeyguard || mForceStatusBarTransparent
                ...
                //Wizos <WizKeyguardAnim> <huangshuiqiang> <2020-12-02> add for start
                //} else if (topIsFullscreen
                } else if ((mPwmExt.isShowWhenLocked( isKeyguardOccluded(), topIsFullscreen, mTopFullscreenOpaqueWindowState) ||topIsFullscreen)
                //Wizos <WizKeyguardAnim> <huangshuiqiang> <2020-12-02> add for end
    ...
    public int focusChangedLw(WindowState lastFocus, WindowState newFocus) {
        ...
        //WizOS WizSystem add 20200922 by zenggz start
        mPwmExt.focusChangedLw(lastFocus, newFocus);
        //WizOS WizSystem add 20200922 by zenggz end
        if ((updateSystemUiVisibilityLw()&SYSTEM_UI_CHANGING_LAYOUT) != 0) {
            // If the navigation bar has been hidden or shown, we need to do another
            // layout pass to update that window.
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_HOME:
                if (isQMMITest && down) {
                    mContext.sendBroadcastAsUser(new Intent("www.qmmi.home.down"), UserHandle.ALL);
                }
                break;
            case KeyEvent.KEYCODE_POWER: {
                if (isQMMITest) {
                    mContext.sendBroadcastAsUser(new Intent("www.qmmi.power.down"), UserHandle.ALL);
                } else {
                    // Any activity on the power button stops the accessibility shortcut
                    cancelPendingAccessibilityShortcutAction();
                    result &= ~ACTION_PASS_TO_USER;
                    isWakeKey = false; // wake-up will be handled separately
                    //WizOS <WizHomeKey> <20201026> add by zenggz start
                    mPowerManager.userActivity(SystemClock.uptimeMillis(), false);
                    //WizOS <WizHomeKey> <20201026> add by zenggz end
                    //modified by cgaopeng for retail demo mode on 20210519 begin
                    if(Settings.Secure.getInt(mContext.getContentResolver(), "vzw_retail_demo_mode", 0) == 1) {
                        if (down && interactive) {
                            mHandler.removeCallbacks(this::reset);
                            mKeyCount++;
                            if (mKeyCount == 4) {
                                mHandler.removeCallbacks(this::reset);
                                reset();
                                ...
                                resetIntent.setClassName("com.dxdragon.retaildemomode", "com.dxdragon.retaildemomode.FactoryReset");
                                resetIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                                mContext.startActivity(resetIntent);
                                break;
                            } else {
                                mHandler.postDelayed(this::reset, 3000);
                            }
                        }
                    }
                    //modified by cgaopeng for retail demo mode on 20210519 begin
                    //add by tdeqin ID1013028 add for handle home event in PostDownloadMessageWindow on 2021-04-18 begin
                    ActivityManager activityManager = (ActivityManager) mContext.getSystemService(Context.ACTIVITY_SERVICE);
                    if (activityManager != null) {
                        List<ActivityManager.RunningTaskInfo> runningTaskInfos = activityManager.getRunningTasks(1);
                        if (runningTaskInfos != null && runningTaskInfos.size() > 0) {
                            ActivityManager.RunningTaskInfo runningTaskInfo = runningTaskInfos.get(0);
                            String className = runningTaskInfo.topActivity.getClassName();
                            android.util.Log.d("ccg", "className=" + className);
                            if ("com.verizon.dmclientupdate.OmadmFumoDialog".equals(className) &&
                                                                    ("1".equals(SystemProperties.get("persist.sys.endcall", "0")) ||
                                                                        "1".equals(SystemProperties.get("system_update_complete","0")))) {
                                result |= ACTION_PASS_TO_USER;
                            //modified by cgaopeng for retail demo mode on 20210519 begin
                            } else if ("com.dxdragon.retaildemomode.RetailDemoModeActivity".equals(className)) {
                                result |= ACTION_PASS_TO_USER;
                                if (hasDownInDemoMode) {
                                    interceptPowerKeyUp(event, interactive, canceled);
                                    hasDownInDemoMode = false;
                                    break;
                                } else if (down && !interactive) {
                                    interceptPowerKeyDown(event, interactive);
                                    hasDownInDemoMode = true;
                                    break;
                                }
                                if (down && interactive) {
                                    // When interactive, we're already awake.
                                ...
                                if(Settings.Secure.getInt(mContext.getContentResolver(), "vzw_retail_demo_mode", 0) == 1) {
                                    //When the screen is off, retaildemomode does not start, press the power key to start
                                    if (down && !interactive) {
                                    ...
                                    if ("com.dxdragon.retaildemomode.FactoryReset".equals(className)) {
                                        result |= ACTION_PASS_TO_USER;
                                        break;
                                    }
                                }
                            }
                            //modified by cgaopeng for retail demo mode on 20210519 end
                        }
                    }
                    //add by tdeqin ID1013028 add for handle home event in PostDownloadMessageWindow on 2021-04-18 end
                    if (down) {
                        interceptPowerKeyDown(event, interactive);
                    } else {
                   //     Log.w(TAG, "hyhhyhkeyeventup");
                        interceptPowerKeyUp(event, interactive, canceled);
                    }
                }
                break;
    ...
    //receiver for mmitest broadcasts add by wyh begin
    BroadcastReceiver qmmiTestReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            Slog.d(TAG,"actoin="+action);
            if (ACTION_QMMITEST_ON.equals(action)) {
         //       Log.w(TAG, "hyhhyhpowerkey");
                isQMMITest = true;

            } else if (ACTION_QMMITEST_OFF.equals(action)) {
          //      Log.w(TAG, "hyhhyhpowerkey2");
                isQMMITest = false;
            }
        }
    };
    ...
    //WizOS <Wiz_QMMI> <zenggz> <2020-11-25> add for start
    private void launchQMMI(Context context) {
        Slog.i(TAG, "launchQMMI +++++ ");
        Intent intent = new Intent();
        intent.setAction("android.intent.action.MAIN");
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        intent.setComponent(new ComponentName("com.qualcomm.qti.qmmi","com.qualcomm.qti.qmmi.framework.CustomMainActivity"));

        List<ResolveInfo> resolves = mContext.getPackageManager().queryIntentActivities(intent,0);
        if(resolves.size()>0){
            mContext.startActivity(intent);
        }else {
            Slog.e(TAG, "com.qualcomm.qti.qmmi/.framework.CustomMainActivity is not exist");
        }
    }
    //WizOS <Wiz_QMMI> <zenggz> <2020-11-25> add for end
}
